...
class ShopeeOrderController extends Controller
{
    ...
    public function __construct(
        ShopeeGetOrderListService $orderService,
    ...
    {
        try {
            Log::info('MANUAL-SYNC-SHOPEE: Sinkronisasi manual dipicu dari controller.');
            ...
            // Ini adalah langkah krusial yang hilang dari controller lama Anda.
            $this->stockSyncService->processPendingOrders();

            // Jika semua berhasil, kembalikan pesan sukses.
            return redirect()->back()->with('success', "Sinkronisasi Shopee berhasil dijalankan. Pesanan baru dan stok sedang diproses.");

        } catch (\Exception $e) {
            // Jika terjadi error di langkah mana pun, catat dan kembalikan pesan error.
            Log::error('MANUAL-SYNC-SHOPEE: Terjadi kesalahan saat sinkronisasi manual.', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            return redirect()->back()->with('error', "Gagal melakukan sinkronisasi Shopee: " . $e->getMessage());
        }
    }
}