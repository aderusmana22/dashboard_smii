...
<div x-show="activeModal === 'produkHabis'" x-cloak style="display: none;" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    {{-- Overlay --}}
    ...
    <div @click.outside="activeModal = ''" class="bg-white rounded-lg shadow-xl overflow-hidden max-w-4xl w-full z-10" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95">
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b">
            <h3 class="text-lg font-bold">Produk Habis</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <template x-if="!modalData.shopee || modalData.shopee.length === 0">
                                    <tr><td colspan="4" class="px-6 py-4 text-center text-gray-500">Tidak ada produk habis di Shopee.</td></tr>
                                </template>
                                <template x-for="product in modalData.shopee" :key="product.id">
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-red-600 font-bold" x-text="product.stock"></div></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><a href="#" class="text-orange-600 hover:text-orange-900">Update Stok</a></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                         ...
                         <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <template x-if="!modalData.tokopedia || modalData.tokopedia.length === 0">
                                    <tr><td colspan="4" class="px-6 py-4 text-center text-gray-500">Tidak ada produk habis di Tokopedia.</td></tr>
                                </template>
                                <template x-for="product in modalData.tokopedia" :key="product.id">
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-500" x-text="product.sku || 'N/A'"></div></td>
                                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-red-600 font-bold" x-text="product.stock"></div></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><a href="#" class="text-green-600 hover:text-green-900">Update Stok</a></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
