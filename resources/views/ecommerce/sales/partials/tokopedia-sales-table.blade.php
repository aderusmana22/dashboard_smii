<div class="overflow-x-auto">
    <table class="min-w-full divide-y divide-gray-200" style="width:100%">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse ($all_tokopedia_sales as $sale)
                <tr>
                     <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600 hover-text-indigo-800"><a href="#">{{ $sale->tiktok_order_id }}</a></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $sale->recipient_name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $sale->created_at_tiktok ? \Carbon\Carbon::parse($sale->created_at_tiktok)->isoFormat('D MMM YYYY, HH:mm') : '-' }}</td>
...
<div class="p-4 border-t" id="tokopedia-pagination-links">
    {{ $all_tokopedia_sales->links() }}
</div>