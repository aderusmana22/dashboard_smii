<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-slate-700">Refinery Configuration</h2>
            <a href="{{ route('oil.config.center') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded hover:bg-gray-200">
                &larr; Back to Center
            </a>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow mb-8 border border-slate-100">
            ...
            <!-- FORM TAMBAH BARU -->
            <form action="{{ route('oil.config.batch_refinery.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                @csrf
                <div class="md:col-span-1">
                    ...
                    <select name="group_name" class="w-full rounded border-slate-300 text-sm" required>
                        ...
                        <option value="Head Tank">5. Head Tank</option>
        ...
        <div class="bg-white rounded-xl shadow border border-slate-100 overflow-hidden">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr>
                        <!-- FORM UPDATE -->
                        <form action="{{ route('oil.config.batch_refinery.update', $tank->id) }}" method="POST">
                            @csrf @method('PUT')
                            <td class="px-4 py-2">
                                <select name="group_name" class="text-xs rounded border-slate-200 bg-slate-50">
                                    @foreach(['Hydro','N.W.B','Deodorizer','Drop Tank','Head Tank','Crystalizer','SX Tank'] as $g)
                        ...
                        <!-- FORM DELETE (Hidden) -->
                        <form id="del-{{$tank->id}}" action="{{ route('oil.config.batch_refinery.destroy', $tank->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                    </tr>
                    @endforeach
                </tbody>
            </table>
