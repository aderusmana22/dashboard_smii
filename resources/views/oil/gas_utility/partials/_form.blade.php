        ...
        <div class="space-y-8 md:space-y-10">
            ...
            @foreach($masters as $gasType => $items)
                @if(!empty($items))
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="px-4 md:px-6 py-4 md:py-6" style="background: {{ $bodyGradient }};">
                            @foreach($items as $item)
                                @include('oil.gas_utility.partials.row_input', ['item' => $item, 'existingReadings' => $existingReadings, 'lastReadings' => $lastReadings])
                            @endforeach
                        </div>
            ...
            <div class="pt-6 md:pt-10 space-y-5">
                ...
                <div class="text-center text-sm md:text-base text-gray-500">
                    Input by: <span class="font-medium">{{ Auth::user()->name ?? 'Operator' }}</span>
                </div>
            </div>
        </div>
    </form>
