...
@if($item->input_type == 'stepper')
    ...
    <div
        class="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 border-b border-dashed border-gray-200 last:border-0">
        ...
        <div class="flex items-center gap-5">
            <!-- Minus -->
            <button type="button" onclick="stepValue({{ $item->id }}, -1)"
            ...
            <input type="number" id="input_{{ $item->id }}" name="readings[{{ $item->id }}]"
                value="{{ intval($stepperValue) }}" readonly data-type="{{ $item->gas_type }}"
                data-min="{{ $item->min_limit }}" data-max="{{ $item->max_limit }}" data-name="{{ $item->name }}"
                class="h-12 w-24 rounded-lg border border-gray-300 bg-white text-center text-xl font-bold text-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition validate-limit">
...
@else
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 border-b border-gray-200 last:border-b-0">
        ...
        <div class="relative w-full sm:w-40">
            <input type="number" step="0.01" name="readings[{{ $item->id }}]" id="input_{{ $item->id }}"
                value="{{ $inputValue }}" placeholder="{{ $placeholderText }}" data-type="{{ $item->gas_type }}"
                data-min="{{ $item->min_limit }}" data-max="{{ $item->max_limit }}" data-name="{{ $item->name }}" class="w-full h-12 text-lg font-bold text-right text-gray-800
                              border border-gray-300 rounded-xl shadow-sm
                              focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500
                              placeholder:text-gray-300 placeholder:font-normal
                              transition bg-slate-50 focus:bg-white validate-limit"
