    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="card rounded-xl shadow-lg overflow-hidden border border-slate-100 h-full flex flex-col">
            <div class="bg-green-700 px-6 py-4 flex justify-between items-center">
                <h4 class="text-white font-semibold text-xl flex items-center gap-2"><i class="mdi mdi-database"></i> Master Data Stock</h4>
                <span id="lastSyncTime" class="card/20 text-white text-base px-2 py-1 rounded"></span>
            </div>
        ...
        <div class="flex flex-col gap-6">
            <div class="card rounded-xl shadow-lg border border-slate-100 p-6">
                <h5 class="text-xl font-bold text-slate-700 mb-4 border-l-4 border-slate-600 pl-3">🔍 Filter Report</h5>
                {{-- FILTER RENTANG TANGGAL BARU --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="stockDateStart" class="block mb-1 text-base font-semibold text-slate-500 uppercase">Tanggal Mulai</label>
                        <input type="date" id="stockDateStart" class="w-full card border-slate-200 text-base rounded-lg p-2.5">
                    </div>
                     <div>
                        <label for="stockDateEnd" class="block mb-1 text-base font-semibold text-slate-500 uppercase">Tanggal Akhir</label>
                        <input type="date" id="stockDateEnd" class="w-full card border-slate-200 text-base rounded-lg p-2.5">
                    </div>
                </div>
                <button type="button" id="btnApplyStockFilter" class="w-full text-white bg-green-700 font-medium rounded-lg text-lg px-5 py-2.5 shadow-md">Tampilkan Tren</button>
            </div>
            <div class="card rounded-xl shadow-lg border border-slate-100 flex-grow">
                <div class="px-6 py-4 border-b border-slate-100 card"><h5 class="font-bold text-slate-700 text-xl">Tren Total Stok Harian per Tipe Minyak</h5></div>
...
$(function () {
    ...
    function updateStockTable(tableData) {
        ...
        tableData.forEach(row => {
            const descBadge = `<span class="px-2 py-1 rounded text-base font-bold ${descColors[row.description] || 'bg-slate-100 text-slate-600'}">${row.description}</span>`;
            tableBody.append(`<tr class="hover:card transition"><td class="px-6 py-3 font-mono text-slate-600 font-semibold">${row.oil_code}</td><td class="px-6 py-3">${descBadge}</td><td class="px-6 py-3 text-right font-mono text-slate-700 font-bold">${new Intl.NumberFormat('id-ID').format(row.current_value)}</td></tr>`);
        });
    }
