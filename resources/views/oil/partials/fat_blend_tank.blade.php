    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-slate-100 h-full flex flex-col">
            <div class="bg-indigo-600 px-6 py-4 flex justify-between items-center">
                <h4 class="text-white font-semibold text-lg flex items-center gap-2"><i class="mdi mdi-blender-software"></i> Data Tangki Blending</h4>
                <span id="fatBlendTotal" class="bg-white/20 text-white text-xs px-2 py-1 rounded"></span>
            </div>
        ...
        <div class="flex flex-col gap-6">
            <div class="bg-white rounded-xl shadow-lg border border-slate-100 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="fatBlendDateStart" class="block mb-1 text-xs font-semibold text-slate-500 uppercase">Tanggal Mulai</label>
                        <input type="date" id="fatBlendDateStart" class="w-full bg-slate-50 border-slate-200 text-sm rounded-lg p-2.5">
                    </div>
                    <div>
                        <label for="fatBlendDateEnd" class="block mb-1 text-xs font-semibold text-slate-500 uppercase">Tanggal Akhir</label>
                        <input type="date" id="fatBlendDateEnd" class="w-full bg-slate-50 border-slate-200 text-sm rounded-lg p-2.5">
                    </div>
                </div>

                {{-- FILTER DROPDOWN LAMA DIHAPUS, GANTI DENGAN TOMBOL --}}
                <button type="button" id="btnApplyFatBlendFilter" class="w-full text-white bg-indigo-600 hover:bg-indigo-700 font-medium rounded-lg text-sm px-5 py-2.5 shadow-md">Tampilkan Tren</button>
            </div>
            <div class="bg-white rounded-xl shadow-lg border border-slate-100 flex-grow">
                <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <h5 class="font-bold text-slate-700">Tren Level Tangki Harian</h5>
...
$(function () {
    ...
    function updateFatBlendTable(tableData) {
        ...
        const sourceBadges = {
            'MANUAL': `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 border border-amber-200"><i class="mdi mdi-pencil"></i> Manual</span>`,
            'PLC': `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200"><i class="mdi mdi-robot-industrial"></i> PLC</span>`,
            'WAITING': `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 border border-gray-200">Waiting</span>`
        };
