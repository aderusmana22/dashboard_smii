    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="card rounded-xl shadow-lg overflow-hidden border border-slate-100 h-full flex flex-col">
            <div class="bg-orange-500 px-6 py-4 flex justify-between items-center">
                <h4 class="text-white font-semibold text-xl flex items-center gap-2"><i class="mdi mdi-package-variant"></i> Stok Packing</h4>
                <span id="packingTotal" class="card/20 text-white text-base px-2 py-1 rounded"></span>
            </div>
        ...
        <div class="flex flex-col gap-6">
            <div class="card rounded-xl shadow-lg border border-slate-100 p-6">
                <h5 class="text-xl font-bold text-slate-700 mb-4 border-l-4 border-orange-500 pl-3">📦 Kontrol Packing</h5>
                {{-- FILTER RENTANG TANGGAL BARU --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="packingDateStart" class="block mb-1 text-base font-semibold text-slate-500 uppercase">Tanggal Mulai</label>
                        <input type="date" id="packingDateStart" class="w-full card border-slate-200 text-base rounded-lg p-2.5">
                    </div>
                    <div>
                        <label for="packingDateEnd" class="block mb-1 text-base font-semibold text-slate-500 uppercase">Tanggal Akhir</label>
                        <input type="date" id="packingDateEnd" class="w-full card border-slate-200 text-base rounded-lg p-2.5">
                    </div>
                </div>
                {{-- Tombol untuk menerapkan filter --}}
                <button type="button" id="btnApplyPackingFilter" class="w-full text-white bg-orange-500 hover:bg-orange-600 font-medium rounded-lg text-lg px-5 py-2.5 shadow-md">Tampilkan Tren</button>
            </div>
            <div class="card rounded-xl shadow-lg border border-slate-100 flex-grow">
                <div class="px-6 py-4 border-b border-slate-100 card flex justify-between items-center">
                    <h5 class="font-bold text-slate-700 text-xl">Tren Volume Harian</h5>
                    <span class="text-base text-orange-600 bg-orange-100 px-2 py-0.5 rounded font-bold">Max: 10,000 Kg</span>
                </div>
...
$(function () {
    ...
    const statusBadges = {
        'READY': '<span class="px-2.5 py-1 rounded-full text-base font-bold bg-green-100 text-green-700">Ready to Pack</span>',
        'FILLING': '<span class="px-2.5 py-1 rounded-full text-base font-bold bg-blue-100 text-blue-700 animate-pulse">Filling...</span>',
        'STANDBY': '<span class="px-2.5 py-1 rounded-full text-base font-bold bg-yellow-100 text-yellow-700">Standby</span>',
        'MAINTENANCE': '<span class="px-2.5 py-1 rounded-full text-base font-bold bg-red-100 text-red-700 border border-red-200">Maintenance</span>'
    };
