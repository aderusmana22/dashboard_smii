<div class="w-full font-sans">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        ...
        <div class="lg:col-span-7 flex flex-col h-full">
            <div class="bg-white rounded-xl shadow-md border border-slate-200 flex flex-col h-[650px]">
                <div class="bg-indigo-600 px-4 py-3 flex justify-between items-center shrink-0">
                    <h4 class="text-white font-semibold text-sm">Data Tank</h4>
                    <span id="totalTankBadge" class="bg-white/20 text-white text-[10px] px-2 py-0.5 rounded-md">Loading...</span>
                </div>
                <div class="overflow-auto flex-grow">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="tableBody" class="divide-y divide-slate-100 text-xs text-slate-700">
                            <tr><td colspan="7" class="p-8 text-center text-slate-400">Pilih filter dan klik tombol untuk menampilkan data.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="lg:col-span-5 flex flex-col gap-6 w-full">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 w-full">
                <div class="flex flex-col sm:flex-row gap-4 items-end w-full">
                    <div class="w-full"><label for="dateStart" class="mb-1 text-[10px] font-bold text-slate-500 uppercase">Tanggal Mulai</label><input type="date" id="dateStart" class="w-full bg-slate-50 border-slate-300 text-sm rounded-lg p-2.5"></div>
                    <div class="w-full"><label for="dateEnd" class="mb-1 text-[10px] font-bold text-slate-500 uppercase">Tanggal Akhir</label><input type="date" id="dateEnd" class="w-full bg-slate-50 border-slate-300 text-sm rounded-lg p-2.5"></div>
                    <div class="w-full"><label for="globalTankSelector" class="mb-1 text-[10px] font-bold text-slate-500 uppercase">Pilih Tangki</label><select id="globalTankSelector" class="w-full bg-slate-50 border-slate-300 text-sm rounded-lg p-2.5"><option value="ALL">🔵 TAMPILKAN SEMUA</option>@foreach($tanks as $tank)<option value="{{ $tank->id }}">{{ $tank->tank_code }}</option>@endforeach</select></div>
                    <button type="button" id="btnApplyFilter" class="w-full sm:w-auto text-white bg-indigo-600 hover:bg-indigo-700 font-medium rounded-lg text-sm px-5 py-2.5 flex items-center justify-center gap-2"><i class="mdi mdi-filter-variant"></i>Filter</button>
                </div>
            </div>
            <div class="bg-white rounded-xl shadow-md border border-slate-200 flex-grow w-full h-[550px] flex flex-col">
                <div class="px-6 py-4 border-b border-slate-100"><h5 class="font-bold text-slate-700">📈 Tren Stok Harian (Kg)</h5></div>
